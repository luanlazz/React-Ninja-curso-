...
it('reduce([1,2], (acc, item) => { acc["number-" + item] = item; return acc }, {}) should return { "number-1": 1, "number-2": 2}', () => {
    expect(reduce([1, 2], (acc, item) => { acc["number-" + item] = item; return acc }, {})).to.be.deep.equal({ "number-1": 1, "number-2": 2 })
})

it('reduce([2, 3, 4], (acc, item, index) => acc + index, 0) should return 3', () => {
    expect(reduce([2, 3, 4], (acc, item, index) => acc + index)).to.be.equal(3)
})

it('reduce([2, 3, 4], (acc, item, index, array) => acc + array[index], 0) should return 9', () => {
    expect(reduce([2, 3, 4], (acc, item, index, array) => acc + array[index])).to.be.equal(9)
})
